...
Colunas gravadas em WA_Analytics:
  sent / delivered / read    -- valores do ponto da serie temporal (USE ESTES no PBI)
  sentTotal / deliveredTotal / readTotal -- total do chunk consultado (referencia apenas)
  startTime / endTime        -- intervalo do ponto (ou do chunk quando delivery sem breakdown)
"""
...
def infer_departamento(name: str) -> str:
    # FIX: guard contra None/vazio antes do .lower() (evita templates sem nome aparecendo como Em branco no PBI)
    if not name:
...
def processar_chunk(tpl: dict, ws: datetime.date, we: datetime.date, analytics_flat: list):
    """
    Busca analytics de um template para um chunk semanal e adiciona as linhas a analytics_flat.

    Regra de startTime (chave do upsert):
    - Com delivery breakdown: usa o startTime retornado pela API em cada ponto
    - Sem breakdown (delivery=[]): usa ws.isoformat() para garantir chave unica por semana
      e evitar que semanas diferentes colidam na chave (templateId, "") e se sobrescrevam.
    """
    tid  = tpl["templateId"]
    ...
    base_row = {
        ...
        # sentTotal = total do chunk (periodo consultado) — nao somar no PBI; usar sent da serie
        "sentTotal":            str(analytics.get("sent") or "0"),
        "deliveredTotal":       str(analytics.get("delivered") or "0"),
        "readTotal":            str(analytics.get("read") or "0"),
    }

    if not delivery:
        # FIX: startTime = inicio do chunk (nao "") para cada semana ter chave unica no upsert
        analytics_flat.append({
            **base_row,
            "sent":      str(analytics.get("sent") or "0"),
            "delivered": str(analytics.get("delivered") or "0"),
            "read":      str(analytics.get("read") or "0"),
            "startTime": ws.isoformat(),
            "endTime":   we.isoformat(),
        ...
        for point in delivery:
            analytics_flat.append({
                **base_row,
                "sent":      str(point.get("sent") or "0"),
                "delivered": str(point.get("delivered") or "0"),
                "read":      str(point.get("read") or "0"),
...
def main():
    ...
    if args.historico:
        ...
        # FIX: modo incremental usa o mesmo padrao de chunking semanal do historico
        # (antes usava uma unica chamada de 7 dias, o que causava inconsistencia de chaves)
        ...
        for tpl in templates_flat:
            print(f"  [{tpl['templateName']}] id={tpl['templateId']}")
            for ws, we in weeks:
                processar_chunk(tpl, ws, we, analytics_flat)
                time.sleep(0.2)
